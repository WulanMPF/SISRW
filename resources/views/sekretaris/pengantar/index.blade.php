                    ...
                    <div class="row">
                        ...
                        <div class="col-md text-right" style="margin-bottom:1rem;">
                            <a class="btn btn-sm mt-1 btn-kembali mr-3" href="{{ url('/sekretaris/surat') }}">Kembali ke
                                menu
                                utama</a>
                            <a class="btn btn-sm mt-1 btn-tambah" href="{{ url('/sekretaris/pengantar/create') }}">+ Buat
                                Surat</a>
                        </div>
                    </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            var dataSuratMasuk = $('#table_surat_pengantar').DataTable({
                serverSide: true,
                ajax: {
                    url: "{{ url('sekretaris/pengantar/list') }}",
                    dataType: "json",
                    type: "POST",
                    data: function(d) {
                        // d.jenis_surat = 'Masuk';
            ...
            $('#confirmationDelete').on('show.bs.modal', function(event) {
                ...
                var form = $('#deleteForm');
                form.attr('action', '{{ url('sekretaris/pengantar/destroy') }}/' + Id);
            });
        });
    </script>
@endpush
